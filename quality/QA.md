...
execution:
  ...
  inputs:
    - type: url
      description: "Target URL of the application to test; auto-detected on feature branches via diff-aware mode"
    - type: description
      description: "Tier (quick/standard/exhaustive), optional auth credentials or cookie file, optional baseline path for regression mode"
  tools-required: [Read, Write, Grep, Glob, Bash, Edit]
  tools-optional: [WebFetch]
  environment: "Requires a running dev server or deployed URL. A clean git working tree so each bug fix can be committed atomically."
discovery:
  goal: "Find, document, and fix bugs in a web application through systematic QA testing with before/after health scoring"
  tasks:
    - "Navigate every page and interact with every element like a real user"
    - "Check console errors after every interaction"
    - "Document each issue with repro steps and screenshots"
    - "Fix bugs in source code with atomic commits"
    - "Re-verify each fix and compute final health score"
  audience:
    roles: ["qa-engineer", "developer", "tech-lead", "product-manager"]
  ...
  triggers:
    ...
    - "run QA"
  not_for:
    - "Unit or integration test writing — use the project's test framework directly"
    - "Performance or load benchmarking — use a dedicated benchmark tool"
    - "Security vulnerability auditing — use a dedicated security audit skill"
---
