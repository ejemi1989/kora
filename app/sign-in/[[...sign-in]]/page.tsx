...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setFormError(null);
    if (!signIn) { setFormError("Sign-in not ready yet"); return; }
    ...
    setFormError(`Unexpected sign-in status: ${status}. Please try again.`);
  };
  ...
  const handleMFAVerify = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    let errorResult: { error: any } | null = null;
    if (useBackupCode) {
      errorResult = await signIn.mfa.verifyBackupCode({ code });
    } else {
      const factors = signIn.supportedSecondFactors;
      if (factors?.some((f) => f.strategy === "phone_code")) {
        errorResult = await signIn.mfa.verifyPhoneCode({ code });
      } else if (factors?.some((f) => f.strategy === "email_code")) {
        errorResult = await signIn.mfa.verifyEmailCode({ code });
      } else {
        errorResult = await signIn.mfa.verifyTOTP({ code });
      }
    }
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div style={{ width: "100%", maxWidth: 420 }}>
          ...
          {step === "mfa" && (
            <>
              <div style={{ marginBottom: 24 }}>
                <h1 style={{ fontSize: 24, fontWeight: 700, letterSpacing: "-0.03em", color: "var(--ink)", margin: 0 }}>Verify your account</h1>
                <p style={{ fontSize: 14, color: "var(--body)", marginTop: 4, marginBottom: 0 }}>Enter the verification code sent to your device</p>
              </div>

              <form onSubmit={handleMFAVerify} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div>
                  <label htmlFor="code" style={{ fontSize: 12, fontWeight: 600, color: "var(--ink)", display: "block", marginBottom: 4 }}>Code</label>
                  <input
                ...
                <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "var(--body)", cursor: "pointer" }}>
                  <input type="checkbox" name="useBackupCode" style={{ width: 16, height: 16 }} />
                  Use backup code
                </label>

                {(errors?.global?.[0] || formError) && (
                  <p style={{ fontSize: 12, color: "var(--danger)", padding: "8px 12px", background: "var(--danger-bg)", borderRadius: "var(--radius-sm)" }}>
              ...
              <div style={{ marginTop: 16, textAlign: "center" }}>
                <button
                  type="button"
                  onClick={() => { signIn.reset(); setStep("sign-in"); }}
